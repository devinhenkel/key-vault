        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
          <thead className="bg-gray-50 dark:bg-gray-800/50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Key Label</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Key Value</th>
              ...
              return (
                <tr key={key.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {key.description && (
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 line-clamp-1 max-w-xs">{key.description}</p>
                    )}
                  </td>

                  {/* Key label */}
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{key.keyLabel}</td>

                  {/* Masked key value */}
                  <td className="px-4 py-3">
                    <code className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 px-2 py-1 rounded font-mono">
                      {key.keyValue}
                    </code>
                  </td>

                  {/* Created by */}
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{key.createdBy}</td>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <button
                      onClick={() => onView(key)}
                      className="text-sm text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 px-2"
                      title="View"
                    >
                      View
                    </button>
                    <button
                      onClick={() => onEdit(key)}
                      className="text-sm text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100 px-2"
                      title="Edit"
                    >
                      Edit
                    </button>
